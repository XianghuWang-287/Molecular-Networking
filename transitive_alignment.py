...
def calculate_N_num(component_list, ratio):
    # Step 1: Sort the list in descending order
    sorted_lengths = sorted(component_list, reverse=True)

    # Step 2: Calculate the total length
    total_length = 699

    # Step 3: Find the N50 length
    cumulative_length = 0
    N_num = 0
    for length in sorted_lengths:
        cumulative_length += length
        if cumulative_length >= total_length * ratio:
            N_num = length
            break

    # Step 4: Return the N50 number
    return N_num
...
if __name__ == '__main__':
    ...
    components_num = len(cast_components)
    plt.figure(figsize=(18, 100))
    for component in cast_components:
        plt.subplot(components_num // 8 + 1, 8, cast_components.index(component) + 1)
    ...
    for component in cast_components:
        num_node_list.append(component.number_of_nodes())
        if component.edges():
            for edge in component.edges():
                output_results.append((edge[0],edge[1],G_all_pairs[edge[0]][edge[1]]['Cosine']))

    num_node_list = sorted(num_node_list, reverse=True)
    print(calculate_N_num(num_node_list,0.2))
